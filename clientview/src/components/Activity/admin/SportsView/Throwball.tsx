...
import { Volleyball as ThrowballModel, Sport, OtherSport } from '@common/models/sports/SportsActivity';
...
export const Throwball = ({ formData, setFormData }: ThrowballProps) => {
  ...
  useEffect(() => {
    ...
    if (!game || Object.keys(game).length === 0) {
      console.log("Game is not initialized, creating new throwball game");
      initializeThrowballGame();
      return;
    }

    // Check for legacy format
    if (isLegacyFormat(game)) {
      console.log("Converting legacy format to throwball format");
      convertLegacyFormat();
      return;
    }

    // Initialize if it's a throwball model but no sets
    if (isThrowballFormat(game) && (!game.sets || game.sets.length === 0) && teams.length >= 2) {
      console.log("Throwball game has no sets, initializing");
      initializeThrowballGame();
      return;
    }
  ...
  // Check if data is in throwball format (using Volleyball model)
  const isThrowballFormat = useCallback((game: any): game is ThrowballModel => {
    // Safety check for undefined or null game
    if (!game) return false;
    return 'sets' in game && Array.isArray(game.sets);
  }, []);

  // Convert from legacy OtherSport format to Throwball format
  const convertLegacyFormat = useCallback(() => {
    try {
      ...
      // Create a throwball model with the sets (using volleyball model)
      const throwballGame = new ThrowballModel();
      throwballGame.sets = initialSets;
      
      setFormData({ ...formData, game: throwballGame } as unknown as SportsActivity<Sport>);
      showNotification("Converted legacy throwball data format", "success");
    } catch (error) {
  ...
  // Initialize a new throwball game
  const initializeThrowballGame = useCallback(() => {
    try {
      console.log("Initializing throwball game with teams:", teams);
      
      if (teams.length < 2) {
        console.log("Not enough teams to initialize throwball game");
        return;
      }
      ...
      console.log("Setting form data with new throwball game:", throwballGame);
      setFormData({ ...formData, game: throwballGame } as unknown as SportsActivity<Sport>);
    } catch (error) {
      console.error("Error initializing throwball game:", error);
    }
  ...
  const updatePoints = useCallback((setIndex: number, teamId: string, newPoints: number) => {
    try {
      if (!isThrowballFormat(game) || !game.sets) return;
      ...
      // Create a throwball model with the updated sets
      const throwballGame = new ThrowballModel();
      throwballGame.sets = updatedSets;
      
      const newFormData = { ...formData, game: throwballGame };
  ...
  const addSet = useCallback(() => {
    ...
    try {
      ...
      if (!isThrowballFormat(game)) {
        console.log("Game is not in throwball format, initializing");
        initializeThrowballGame();
        return;
      }
  ...
  }, [formData, game, teams, setFormData, showNotification, initializeThrowballGame, setExpanded]);

  // Delete a set
  const deleteSet = useCallback((setIndex: number) => {
    try {
      if (!isThrowballFormat(game)) return;
  ...
  const getTeamTotalPoints = useCallback((teamId: string) => {
    if (!isThrowballFormat(game) || !game.sets) return 0;
      ...
      <Paper elevation={1} sx={{ p: 3, borderRadius: 2 }}>
        <Box sx={{ textAlign: 'center', py: 4 }}>
          <Typography variant="h6" color="text.secondary">
            At least two teams are required to set up the throwball match.
          </Typography>
        </Box>
      </Paper>
    );
  ...
  // If game data is not in throwball format and not yet converted, show loading
  if (!isThrowballFormat(game) && !isLegacyFormat(game)) {
    return (
      <Paper elevation={1} sx={{ p: 3, borderRadius: 2 }}>
        <Box sx={{ textAlign: 'center', py: 4 }}>
          <Typography variant="h6" color="text.secondary">
            Initializing throwball match data...
          </Typography>
        </Box>
      </Paper>
    );
    ...
    <Paper elevation={1} sx={{ p: 3, borderRadius: 2 }}>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" fontWeight="bold" sx={{ display: 'flex', alignItems: 'center', color: 'primary.main' }}>
          <SportsVolleyballIcon sx={{ mr: 1 }} /> Throwball Match
        </Typography>
        <Divider sx={{ mt: 1 }} />
      </Box>
      ...
      {/* Sets as Accordions */}
      {isThrowballFormat(game) && game.sets && game.sets.map((set, index) => (
        <Accordion 
          key={`set-${index}`}
...
export const ThrowballForm = Throwball;
